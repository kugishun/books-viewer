...
const Home = () => {
  ...
  const LogInUserList = (props) => {
    const { bookList } = props;

    return (
      <div className="bookList" key={bookList.id}>
        <div onClick={() => moveDetail(bookList)}>
          <p className="bookList__title font-size-L">{bookList.title}</p>
        </div>
        <p className="bookList__url font-size-S">
          URL:{" "}
        ...
        <Container>
          ...
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="me-auto">
              {auth ? <Nav.Link onClick={newPost}>新規登録</Nav.Link> : <></>}
            </Nav>
          </Navbar.Collapse>
          <Nav className="me-auto">
            {auth ? (
              <NavDropdown title={userName}>
                <NavDropdown.Item onClick={setting}>
                  <img src={userIcon} width={50} height={50} alt="userIcon" />
                </NavDropdown.Item>
      ...
      <div className="Home">
        <div className="popup-menu-container">
          <div className={`popup-menu ${checkPopOut ? "shown" : ""}`}>
            ...
            <div>
              <Button className="popup-menu__button" onClick={logOut}>
                ログアウト
              </Button>
              <Button className="popup-menu__button" onClick={cancelButton}>
                キャンセル
              </Button>
            </div>
          </div>
        </div>
        <div className="Home-bookLists">
          {bookLists.map((bookList) => {
            return auth ? (
              <LogInUserList bookList={bookList} />
            ) : (
              <GestUserList bookList={bookList} />
            );
          })}
        </div>
